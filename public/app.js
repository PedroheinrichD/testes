const state = { id: '', name: '', roomId: '', socket: null, localStream: null, previewStream: null, cameraTrack: null, cameraFacing: 'user', screenTrack: null, screenStream: null, peers: new Map(), iceServers: [], sharing: false, audioContext: null, analyser: null, speaking: false, speakingFrame: 0 };
...
function addCameraSwitcher(tile) { if (!tile || tile.querySelector('.camera-switcher')) return; const button = document.createElement('button'); button.className = 'camera-switcher'; button.type = 'button'; button.title = 'Trocar câmera frontal/traseira'; button.setAttribute('aria-label', 'Trocar câmera frontal/traseira'); button.textContent = '⟳'; button.addEventListener('click', switchCamera); tile.append(button); }
...
function makeTile(id, name, stream, local = false, participant = {}) {
  ...
  if (stream?.getVideoTracks().length) { const video = document.createElement('video'); video.autoplay = true; video.playsInline = true; video.muted = local; video.srcObject = stream; tile.append(video); if (local) addCameraSwitcher(tile); }
...
async function getMedia() {
  ...
  try { return await navigator.mediaDevices.getUserMedia({ audio: audioConstraints, video: { facingMode: { ideal: state.cameraFacing } } }); }
  catch (error) {
...
async function enableCamera() { try { const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: state.cameraFacing } } }); const track = stream.getVideoTracks()[0]; if (!track) throw new Error('camera-unavailable'); state.localStream.addTrack(track); state.cameraTrack = track; updateTile('local', state.localStream, state.name); updateTileStatus($('#tile-local'), { mic: state.localStream.getAudioTracks()[0]?.enabled !== false, camera: true }); updateParticipantChip('local', state.name, { mic: state.localStream.getAudioTracks()[0]?.enabled !== false, camera: true }, true); $('#toggle-camera').classList.add('active'); $('#toggle-camera').classList.remove('muted'); for (const [peerId, peer] of state.peers.entries()) { const sender = peer.connection.getSenders().find(item => item.track?.kind === 'video' || item.kind === 'video'); if (sender && !peer.screenSender) await sender.replaceTrack(track); else if (!sender) { peer.connection.addTrack(track, state.localStream); await negotiatePeer(peerId, peer); } } updateCallStageMode(); send({ type: 'state', camera: true }); } catch { showNotice('Não foi possível ativar a câmera. Verifique a permissão do navegador.'); } }
async function getCameraStream(facing) { try { return await navigator.mediaDevices.getUserMedia({ video: { facingMode: { exact: facing } } }); } catch { return navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: facing } } }); } }
async function switchCamera() { if (!navigator.mediaDevices?.getUserMedia) { showNotice('Seu navegador não permite trocar a câmera.'); return; } if (!state.cameraTrack) { await enableCamera(); return; } const nextFacing = state.cameraFacing === 'user' ? 'environment' : 'user'; try { const stream = await getCameraStream(nextFacing); const nextTrack = stream.getVideoTracks()[0]; if (!nextTrack) throw new Error('camera-unavailable'); const previousTrack = state.cameraTrack; state.localStream.removeTrack(previousTrack); state.localStream.addTrack(nextTrack); state.cameraTrack = nextTrack; state.cameraFacing = nextFacing; previousTrack.stop(); updateTile('local', state.localStream, state.name); for (const peer of state.peers.values()) { const sender = peer.connection.getSenders().find(item => item.track?.kind === 'video' || item.kind === 'video'); if (sender && !peer.screenSender) await sender.replaceTrack(nextTrack); } updateCallStageMode(); } catch { showNotice('Não foi possível trocar para a outra câmera.'); } }
...
async function switchPreviewCamera() { const currentTrack = state.previewStream?.getVideoTracks()[0]; if (!currentTrack) { showNotice('Nenhuma câmera está disponível para trocar.'); return; } const nextFacing = state.cameraFacing === 'user' ? 'environment' : 'user'; try { const stream = await getCameraStream(nextFacing); const nextTrack = stream.getVideoTracks()[0]; if (!nextTrack) throw new Error('camera-unavailable'); state.previewStream.removeTrack(currentTrack); state.previewStream.addTrack(nextTrack); state.cameraFacing = nextFacing; currentTrack.stop(); $('#preview-video').srcObject = state.previewStream; } catch { showNotice('Não foi possível trocar para a outra câmera.'); } }
...
$('#preview-switch-camera').addEventListener('click', async () => { await switchPreviewCamera(); });
