...
function avatarMarkup(id, name) { return `<img src="${avatarUrl(id, name)}" alt="Avatar de ${escapeHtml(name)}" loading="lazy"><span>${initials(name)}</span>`; }
function setAvatar(element, id, name) { element.innerHTML = avatarMarkup(id, name); const image = element.querySelector('img'); image.addEventListener('error', () => { image.remove(); element.querySelector('span').style.display = 'block'; }, { once: true }); }
