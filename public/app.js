...
async function switchPreviewCamera() { const currentTrack = state.previewStream?.getVideoTracks()[0]; if (!currentTrack) { showNotice('Nenhuma câmera está disponível para trocar.'); return; } const nextFacing = state.cameraFacing === 'user' ? 'environment' : 'user'; try { const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: nextFacing } } }); const nextTrack = stream.getVideoTracks()[0]; if (!nextTrack) throw new Error('camera-unavailable'); state.previewStream.removeTrack(currentTrack); state.previewStream.addTrack(nextTrack); state.cameraFacing = nextFacing; currentTrack.stop(); $('#preview-video').srcObject = state.previewStream; } catch { showNotice('Não foi possível trocar para a outra câmera.'); } }
